...
function ExpressHandlebars(config) {
    ...
    if ('sharedPartialsDir' in config) { this.sharedPartialsDir = config.sharedPartialsDir; }
...
extend(ExpressHandlebars.prototype, {
    ...
    sharedPartialsDir: null,

    // -- Public Methods -------------------------------------------------------

    loadPartials: function (filePath, options, callback) {
        ...
        async.waterfall([
            load.bind(this, filePath, options),
//            load.bind(this, this.partialsDir, options),
    ...
    render: function (filePath, options, callback) {
        ...
        function loadTemplates(callback) {
            var templates = {
                partials: this.loadPartials.bind(this, this.partialsDir, options),
                template: this.loadTemplate.bind(this, filePath, options)

            };

            if(this.sharedPartialsDir) {
                templates.sharedPartials = this.loadPartials.bind(this, this.sharedPartialsDir, options);
            }
            async.parallel(templates, callback);
        }

        function renderTemplate(templates, callback) {
            this._renderTemplate(templates.template, options, {
                helpers : helpers,
                partials: _.extend(templates.sharedPartials || {}, templates.partials)
            }, callback);
        }
